...
<div class="siddhi-home-page">
	...
	<div class="ecommerce-content multi-vendore-content">
		...
		<section class="top-categories categories_items">
		    
        </section>

		<section class="popular-section d-none">
		    <div class="container">
		        <div class="title d-flex align-items-center">
		...
		<section class="new-arrivals">
		    <div class="container">
		        <div class="title d-flex align-items-center">
		            ...
		            <span class="see-all ml-auto">
		                <a href="{{url('products')}}">{{trans('lang.see_all')}}</a>
		            </span>
		        </div>
		...
		<section class="home-categories d-none">
...
<script type="text/javascript">
	...
	function buildHTMLHomeCategoryStores(category_id) {
        ...
        var storeHtmlRes = vendorCatRef.get().then(async function (nearestRestauantSnapshot) {
            ...
            alldata.forEach((listval) => {
                ...
                html = html + '<div class="col-md-3 product-list"><div class="list-card position-relative"><div class="list-card-image">';
                    if(val.photo)   photo   =   val.photo;
                    else            photo   =   placeholderImageSrc;
    ...
    function buildHTMLItemCategory(foodCategories) {
        ...
        alldata.forEach((listval) => {
            ...
            trending_route = trending_route.replace('restaurants', 'products');
            ...
            if(val.title == 'Waffle' || val.title == 'Crepes' || val.title == 'Cold Drinks' || val.title == 'Hot Drinks'){
                var refCategoryItem = database.collection('vendor_products').where("categoryID", "==", category_id).where("publish","==",true).limit(4);
                refCategoryItem.get().then(async function (snapshotsCategoryItem) {
                    var trendingCategoryItemhtml = buildHTMLCategoryItem(snapshotsCategoryItem);
                    
                    var categories_items_html = '<div class="container" style="margin-top:40px;">';
        		        categories_items_html += '<div class="title d-flex align-items-center">';
        		            categories_items_html += '<h5 id="categories_items_name">'+val.title+'</h5>';
        		            categories_items_html += '<span class="see-all ml-auto"><a href="'+trending_route+'">See All</a></span>';
                        categories_items_html += '</div>';
                        categories_items_html += '<div class="append_categories">'+trendingCategoryItemhtml+'</div>';
                    categories_items_html += '</div>';
                    
                    $('.categories_items').append(categories_items_html);
                });
            }
        });
        html += '</div>';
    ...
    function buildHTMLCategoryItem(snapshotsCategoryItem){
        var html = ''; 
        var alldata = [];
        snapshotsCategoryItem.docs.forEach((listval) => {
            var datas = listval.data();
            datas.id = listval.id;
            alldata.push(datas);
        });
	...
	async function getCouponsList() {
		//if (popularStoresList.length > 0) {
			...
			var couponsRef2 = database.collection('coupons').where('isEnabled', '==' ,true).orderBy("expiresAt").startAt(new Date());
			couponsRef2.get().then(async function (couponListSnapshot) {
			    ...
			    console.log(couponListSnapshot);
	...
	function buildHTMLCouponList(couponListSnapshot) {
        ...
        alldata.forEach((listval) => {
            ...
            html=html+ '<div class="transactions-list-wrap mt-4"><div class="bg-white px-4 py-3 border rounded-lg mb-3 transactions-list-view shadow-sm"><div class="gold-members d-flex align-items-center transactions-list">';
                ...
                html=html+'</div><div class="float-right ml-auto"><span class="price font-weight-bold h4">'+price_val+'</span></div></div>';
